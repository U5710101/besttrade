...
def get_investors_by_name(): # expects query parameter name, if not available it will get all investors
    try:
        investors = []
        name = request.args.get('name')
        investors = investordao.get_all_investors() if name is None else investordao.get_investor_with_name(name)
        res = make_response()
        res.response = json.dumps([investor.__dict__ for investor in investors])
        res.headers['Content-Type'] = 'application/json'
        res.status = HTTPStatus.OK
